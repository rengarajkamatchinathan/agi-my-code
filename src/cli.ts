...
async function bootstrap(
  ...
  /** Skills loaded at startup — passed to App for synchronous slash-picker population. */
  loadedSkills: import("./types/command.js").PromptCommand[];
  ...
  // ─── Skills (Phase 4.2) ───
  // Load skills and register as BOTH AI tools AND slash commands so the TUI
  // slash-picker shows them. This matches the beta pattern where getCommands()
  // is the single source of truth for both dispatch and autocomplete.
  let _loadedSkills: import("./types/command.js").PromptCommand[] = [];
  try {
    ...
    // 1. Register as AI tools (unchanged)
