...
import { createCommandRegistry, type CommandRegistry, type CommandContext } from "../commands/index.js";
...
interface Props {
  ...
  /**
   * Skills loaded at startup by the CLI's bootstrap() — pre-populates the
   * slash-picker synchronously so skills appear on the very first keystroke.
   * When absent (Electron / dev server) the async IIFE in commandRegistry
   * will populate them after a brief delay.
   */
  initialSkills?: import("../types/command.js").PromptCommand[];
}
...
function AppInner({
  ...
  // Bumped once after the async skill/MCP loading IIFE completes so RichInput
  // re-derives its command list (SlashCommand[]) from the populated registry.
  ...
  const commandRegistry = useMemo(() => {
    ...
    // If initialSkills were pre-loaded by the CLI bootstrap, register them first
    // (nearly instant — no disk I/O) so the slash picker is populated immediately.
    // Then a full loadSkills(cwd) follows in case Electron has skills the CLI missed.
    (async () => {
      try {
        const { loadSkills, skillToCommand } = await import("../skills/index.js");

        // Pre-seed from CLI-loaded skills (synchronous from caller's perspective).
        if (initialSkills && initialSkills.length > 0) {
          for (const skill of initialSkills) {
            reg.register(skillToCommand(skill));
          }
          // Bump immediately so RichInput shows these skills without waiting for disk.
          setSkillsVersion((v) => v + 1);
        }

        // Full disk load — catches Electron-side skills not passed via initialSkills.
        const skills = await loadSkills(cwd);
        for (const skill of skills) {
          // register is idempotent for same-name skills (last write wins)
          reg.register(skillToCommand(skill));
        }
      } catch {}
      ...
      // Final bump: includes any MCP prompts + any skills added during disk load.
      setSkillsVersion((v) => v + 1);
    })();
    return reg;
  ...
  async function handleSlash(raw: string): Promise<void> {
    ...
    (ctx as any)._fileHistory = (globalThis as any).__myCodeFileHistory;
    (ctx as any)._transcript = transcript;
    (ctx as any)._loadedPlugins = (globalThis as any).__myCodeLoadedPlugins;
  ...
  // Derive the slash-command list from the live registry for RichInput.
  // Re-computed whenever the registry is populated (skillsVersion bump).
  // skillsVersion is the reactive trigger — commandRegistry is mutable and
  // won't change identity, but skillsVersion bumps after skills are loaded.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const slashCommandsList = useMemo<SlashCommand[]>(
    () =>
      commandRegistry.list().map((def) => ({
        name: def.name,
        desc: def.description,
        args: def.argsHint,
      })),
    [commandRegistry, skillsVersion] // eslint-disable-line react-hooks/exhaustive-deps
  );
