...
import { filterCommands, filterCommandsFromRegistry } from "./slashCommands.js";
...
interface Props {
  ...
  cwd?: string;
  /**
   * Live command list from App's commandRegistry (includes skills + MCP prompts).
   * When provided, the slash picker uses this instead of the static built-in list.
   * Derived via useMemo in App.tsx and passed down — updates whenever skills load.
   */
  commands?: import("./slashCommands.js").SlashCommand[];
}
...
export function RichInput({ onSubmit, disabled, placeholder, editMode, bypassAll, cwd, commands: commandsProp }: Props) {
  ...
  // Use the live commands prop (from App's registry — includes skills) if available,
  // otherwise fall back to the static built-in list (builtins only, no cwd, no skills).
  const matches = showingPicker
    ? commandsProp !== undefined
      ? commandsProp.filter((c) => {
          const q = slashQuery.toLowerCase().trim();
          if (!q) return true;
          const starts = c.name.toLowerCase().startsWith(q);
